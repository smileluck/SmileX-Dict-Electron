...
export default function SpeakButton({ text, mode = 'word', size = 20, className = '' }: SpeakButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className={`inline-flex items-center justify-center transition-colors ${speaking ? 'text-brand-500' : 'text-gray-400 hover:text-brand-500'} ${className}`}
      title={mode === 'word' ? '朗读单词' : mode === 'sentence' ? '朗读句子' : '朗读中文'}
    >
      {speaking ? (
