    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1">
            <div className="flex items-center justify-between text-sm mb-1">
              <span className="text-gray-500">写作词汇掌握度</span>
              <span className="text-brand-600 font-medium">{collectionStats.progress}%</span>
            </div>
          ...
          <div className="text-right text-sm text-gray-500">
            <span className="text-green-600 font-medium">{collectionStats.known}</span> / {collectionStats.total}
          </div>
        </div>
      </div>
      ...
      {!showPrompts ? (
        <>
          ...
          {currentCollection && (
            <div className="text-sm text-gray-600 px-1">{currentCollection.description}</div>
          )}
                ...
                return (
                  <div
                    ...
                    className={`rounded-lg border p-4 transition-colors ${
                      isMastered ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200'
                    }`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        {word.examples.map((ex, idx) => (
                          <p key={idx} className="text-sm text-gray-500 italic mb-1">{ex}</p>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-100 space-y-2">
                      <div>
                        <span className="text-xs text-gray-500">同义表达：</span>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {word.synonyms.map(syn => (
                            <span key={syn} className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded">
                              {syn}
                            </span>
                          ))}
                        </div>
                      ...
                      <div>
                        <span className="text-xs text-gray-500">可替换：</span>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {word.replacements.map(rep => (
                            <span key={rep} className="text-xs bg-red-50 text-red-600 px-2 py-0.5 rounded line-through">
                              {rep}
                            </span>
                          ))}
        ...
        /* Writing prompts */
        <div className="space-y-3">
          <h4 className="font-medium text-gray-700">写作练习题目</h4>
          {WRITING_PROMPTS.map(prompt => (
            <div key={prompt.id} className="rounded-lg border bg-white p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
              ...
              <div className="mt-3 pt-3 border-t border-gray-100">
                <span className="text-xs text-gray-500">需要使用的词汇：</span>
                <div className="flex flex-wrap gap-1 mt-1">
                  {prompt.targetVocabulary.map(v => (
                    <span key={v} className="text-xs bg-brand-50 text-brand-700 px-2 py-0.5 rounded">
                      {v}
                    </span>
                  ))}
                </div>
